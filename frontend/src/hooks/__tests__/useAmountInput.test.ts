...
describe('useAmountInput', () => {
  describe('基本機能', () => {
    ...
    it('初期値が正の数値の場合、フォーマットされた文字列が返される', () => {
      ...
      expect(result.current[0]).toBe('¥15,000')
    })
    ...
    it('setValue関数で値を更新すると、フォーマット済み文字列が更新される', () => {
      ...
      expect(result.current[0]).toBe('¥50,000')
    })
  })
  ...
  describe('フォーマット処理', () => {
    ...
    it('1000以上の数値はカンマ区切りでフォーマットされる', () => {
      ...
      expect(result.current[0]).toBe('¥1,234,567')
    })
  ...
  describe('数値更新処理', () => {
    ...
    it('複数回の値更新が正しく反映される', () => {
      ...
      expect(result.current[0]).toBe('¥2,000')

      // 2回目の更新
      act(() => {
        result.current[1](3500)
      })
      expect(result.current[0]).toBe('¥3,500')
    })
  })
  ...
  describe('エッジケース', () => {
    ...
    it('非常に大きな数値も正しくフォーマットされる', () => {
      ...
      expect(result.current[0]).toBe('¥999,999,999,999')
    })
  })
