...
function generateStoryFile(component) {
  ...
  const relativePath = path.relative(path.dirname(filePath), filePath).replace('.tsx', '');
...
function getDefaultValue(type) {
  ...
  if (type.includes('function') || type.includes('=>')) return '() => console.log("clicked")';
  return undefined;
}
...
async function main() {
  ...
  const componentFiles = findFiles(CONFIG.componentsPattern, CONFIG.excludePatterns);
