...
describe('AppTopBar', () => {
  ...
  it('基本的なレンダリングが正しく動作する', () => {
    ...
    // タイトルが表示される
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('家計簿アプリ')
  ...
  it('カスタムタイトルが正しく表示される', () => {
    renderWithTheme(
      <AppTopBar {...defaultProps} title="My Budget App" />
    )
    
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('My Budget App')
  })
  ...
  it('長いタイトルでもnoWrapで正しく表示される', () => {
    ...
    expect(title).toHaveTextContent('ファミリー家計簿管理システム - 収支管理アプリケーション')
  ...
  it('ハンバーガーメニューボタンがクリック可能', async () => {
    ...
    const menuButton = screen.getByRole('button', { name: 'ナビゲーションメニューを開く' })
  ...
  it('アクセシビリティ属性が正しく設定される', () => {
    ...
    const menuButton = screen.getByRole('button', { name: 'ナビゲーションメニューを開く' })
    expect(menuButton).toHaveAttribute('aria-label', 'ナビゲーションメニューを開く')
  ...
  it('複数のプロパティが同時に適用される', async () => {
    ...
    // タイトルの確認
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('カスタムアプリ')
