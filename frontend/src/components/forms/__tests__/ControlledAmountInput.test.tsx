...
describe('ControlledAmountInput', () => {
  it('プレースホルダーが表示される', () => {
    ...
    expect(screen.getByPlaceholderText('金額を入力してください')).toBeInTheDocument()
  })
  ...
  it('undefinedの値が数値として正しく変換される', () => {
    render(<TestFormWrapper defaultValues={{ amount: undefined as any }} />)
    const input = screen.getByPlaceholderText('金額を入力してください')
    expect(input).toHaveValue('')
  })

  it('nullの値が数値として正しく変換される', () => {
    render(<TestFormWrapper defaultValues={{ amount: null as any }} />)
    const input = screen.getByPlaceholderText('金額を入力してください')
    expect(input).toHaveValue('')
  })

  it('文字列の値が数値として正しく変換される', () => {
    render(<TestFormWrapper defaultValues={{ amount: '1500' as any }} />)
    const input = screen.getByPlaceholderText('金額を入力してください')
