 ...
 * グローバルなテスト最適化設定
 *
 * 注意: グローバルでのフェイクタイマー有効化は、
 * 非同期挙動（MUIの遅延処理やRTLのwaitFor等）をブロックし、
 * テストが完了しない原因となるため無効化します。
 * フェイクタイマーが必要なテストは各テスト内で明示的に
 * `jest.useFakeTimers()` / `jest.useRealTimers()` を呼び出してください。
 */
// ここでは実タイマーを使用する（デフォルト）
