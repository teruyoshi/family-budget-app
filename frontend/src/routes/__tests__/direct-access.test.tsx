  ...
  describe('Navigation State Consistency', () => {
    test('navigation highlights correct item for direct access', async () => {
      ...
      for (const { path, expectedActive } of testCases) {
        ...
        await waitFor(() => {
          const activeMenuItem = screen.getByRole('menuitem', { name: expectedActive })
          expect(activeMenuItem).toHaveClass('Mui-selected')

          // 他の項目が選択されていないことを確認
          const allMenuItems = screen.getAllByRole('menuitem')
          const otherItems = allMenuItems.filter(item => 
            item.getAttribute('aria-label') !== expectedActive
          )
    ...
    test('app layout is consistent across direct access', async () => {
      ...
      for (const path of paths) {
        ...
        await waitFor(() => {
          ...
          // MUIコンテナが存在することを確認
          const container = screen.getByText(/家計簿アプリ|支出管理|収入管理|履歴表示|設定/).closest('[class*="MuiContainer"]')
    ...
    test('code splitting works properly for direct access', async () => {
      ...
      for (const page of pages) {
        ...
        await waitFor(() => {
          ...
          const activeItems = screen.getAllByRole('menuitem').filter(item =>
    ...
    test('graceful handling of malformed URLs', async () => {
      ...
      for (const url of malformedUrls) {
        ...
        await waitFor(() => {
          ...
          const isNotFound = screen.queryByText('404 - ページが見つかりません')
