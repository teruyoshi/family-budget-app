...
describe('IncomeInput', () => {
  test('正の整数の入力が許可される', () => {
    ...
    render(<IncomeInput value="" onChange={mockOnChange} />)

    const input = screen.getByPlaceholderText('収入金額を入力')
  ...
  test('不正な文字の入力が無視される', () => {
    ...
    render(<IncomeInput value="100" onChange={mockOnChange} />)
    ...
    // 不正な値の場合、onChangeが呼ばれない（IncomeInputの仕様）
  ...
  test('複数の小数点入力が制御される', () => {
    ...
    render(<IncomeInput value="12.34" onChange={mockOnChange} />)
    ...
    // 不正な形式の場合、onChangeが呼ばれない（IncomeInputの仕様）
