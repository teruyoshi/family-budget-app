...
function generateDefaultArgs(props) {
  ...
  const args = props
    .filter(
      (prop) => !prop.optional || ['placeholder', 'label'].includes(prop.name)
    )
    .map((prop) => {
...
function getDefaultValue(type, propName) {
  if (type.includes('string')) {
    if (propName.toLowerCase().includes('placeholder'))
      return 'プレースホルダーテキスト'
  ...
  if (type.includes('number')) {
    if (
      ...
      propName.toLowerCase().includes('balance')
    )
      return 1000
...
function generateStoryTemplate(component, category) {
  ...
  const requiredPropsString = requiredProps
    .map((prop) => {
...
export const Variants: Story = {
  ...
  // 一般的なバリエーション（MUI TextFieldなど）
    ...
    props.some((p) => p.name === 'variant' && p.type.includes('outlined'))
  ) {
    variants.push(`
export const Variants: Story = {
  render: () => (
...
function main(options = {}) {
  ...
  console.log(`\\n🎉 完了！`)
  console.log(
    `📊 生成: ${generatedCount}件, 更新: ${updatedCount}件, スキップ: ${skippedCount}件`
  )
  console.log(`\\n🔧 使用方法:`)
  console.log(
    `   make generate-stories-frontend        # ストーリー自動生成（差分更新）`
  )
  console.log(
    `   make generate-stories-frontend-force  # ストーリー強制再生成（全更新）`
  )
